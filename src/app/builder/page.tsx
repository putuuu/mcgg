...
export default function SynergyBuilder() {
  const [board, setBoard] = useState<(Hero | null)[]>(Array(21).fill(null)); // 3x7
  ...
  const activeSynergies = useMemo(() => {
    ...
    board.forEach((hero) => {
      if (!hero) return;
      if (uniqueHeroes.has(hero.id)) return;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const hero = active.data.current?.hero as Hero | undefined;
    ...
    if (!over && source === "board" && fromIndex !== undefined) {
      const next = [...board];
      next[fromIndex] = null;
      setBoard(next);
    }
  ...
  const filteredHeroes = useMemo(() => {
    return heroes.filter((h) => {
      ...
      if (search && !h.name.toLowerCase().includes(search.toLowerCase()))
        return false;
      if (factionFilter && h.synergies.faction !== factionFilter) return false;
                              ...
                              >
                                ...
                                <div className="absolute z-50 hidden group-hover:block left-24 top-0 w-64 bg-gray-900/95 text-white rounded-md shadow-lg p-3">
                                  ...
                                  {/* <p className="text-sm text-gray-400 mb-1">
                                    Mana: {cell.mana.initial}/{cell.mana.max}
                                  </p> */}
                ...
                {({
                  ...
                  >
                    ...
                    <div className="absolute z-50 hidden group-hover:block left-20 top-0 w-64 bg-gray-900/95 text-white rounded-md shadow-lg p-3">
                      ...
                      {/* <p className="text-sm text-gray-400 mb-1">
                          Mana: {hero.mana.initial}/{hero.mana.max}
                        </p> */}
